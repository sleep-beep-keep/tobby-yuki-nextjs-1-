...
import { CheckCircle2, Heart, MapPin, Minus, Plus } from "lucide-react";
...
export default function ProductActions({ product, variants }: { product: Product; variants: ProductVariant[] }) {
  ...
  const maximumQuantity = hasSizes ? selectedVariant?.stockQuantity ?? 0 : 20;
  ...
  return <>
    ...
    <div className="mt-7"><p className="text-sm font-semibold">Quantity</p><div className="mt-3 inline-flex items-center rounded-full border border-black/10 bg-white"><button type="button" aria-label="Decrease quantity" disabled={quantity <= 1} onClick={() => setQuantity((current) => Math.max(1, current - 1))} className="p-3 text-cocoa disabled:cursor-not-allowed disabled:opacity-35"><Minus size={16} /></button><span className="w-9 text-center text-sm font-semibold" aria-live="polite">{quantity}</span><button type="button" aria-label="Increase quantity" disabled={!canAddToCart || quantity >= maximumQuantity} onClick={() => setQuantity((current) => Math.min(maximumQuantity, current + 1))} className="p-3 text-cocoa disabled:cursor-not-allowed disabled:opacity-35"><Plus size={16} /></button></div>{hasSizes && selectedVariant && <p className="mt-2 text-xs text-gray-500">Maximum {selectedVariant.stockQuantity} for this size.</p>}</div>
    <div className="mt-8 flex gap-3"><button disabled={!canAddToCart} onClick={() => addItem(product, hasSizes ? size : undefined, quantity)} className="flex-1 rounded-full bg-cocoa py-4 font-bold text-white transition hover:bg-mocha disabled:cursor-not-allowed disabled:opacity-40">{hasSizes && !selectedVariant ? "Select a size" : "Add to Cart"}</button><button aria-label="Add to wishlist" className="rounded-full border p-4"><Heart /></button></div>
